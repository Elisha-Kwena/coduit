// components/Sidebar.tsx (or wherever your Sidebar lives)
"use client"
...
export default function Sidebar() {
    const { isOpen, toggleSidebar } = useSidebar(); // ← Now we get toggleSidebar too
    ...
    const sidebarRef = useRef<HTMLElement>(null) // ← Ref for the sidebar itself
    ...
    // Close bottom widgets when clicking outside them
    useEffect(() => {
    ...
    useEffect(() => {
        // Only activate when sidebar is open
        if (!isOpen) return;
        ...
        const handleMobileClickOutside = (event: MouseEvent) => {
            if (
                ...
                window.innerWidth < 768 // mobile only (Tailwind md = 768px)
            ) {
    ...
    return (
        <>
            <DropdownProvider>
                <aside
                    ref={sidebarRef}
                    className={`w-72 h-screen bg-white dark:bg-black p-3 pb-1 md:pt-20 pt-24 md:block fixed top-0 transition-all duration-300 ease-in-out ${
                        isOpen ? "left-0" : "md:left-0 -left-72"
                    } z-50`}
                >
                    <div className="w-full h-full flex items-center justify-between flex-col gap-4">
                            ...
                            <div className="nav-item w-full h-30">
                                <Link
                                    ...
                                    className={`block pl-1 p-2 rounded font-fira-code ${
                                        pathname === '/feeds'
                                            ? 'text-sapphire dark:bg-[#232323] shadow bg-gray-500'
                                            : 'dark:text-white bg-gray-200 dark:bg-black'
                                    }`}>
                        ...
                        {/* bottom side */}
                        <div ref={bottomsideRef} className="w-full shrink-0 h-30 relative flex items-center justify-end flex-col">
